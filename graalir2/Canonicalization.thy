...
proof -
  have zv: "g m \<turnstile> z (kind g z) \<mapsto> IntVal(xv+yv)"
    using AddNode eval_const_node xn yn zn by simp
...
proof -
  ...
  have refx: "g m \<turnstile> zx (RefNode x) \<mapsto> IntVal xv"
    using RefNode xn xv by blast 
  have refy: "g m \<turnstile> zy (RefNode y) \<mapsto> IntVal yv"
    using RefNode yn yv by blast 
      ...
      proof -
        ...
        next
          ...
          then show ?thesis
            sorry
        qed
        then show ?thesis using cv2 ae xvv yn yv RefNode by auto
      qed
    qed
  next
    ...
    proof (cases "kind g y = ConstantNode yvv")
      ...
      have cv3: "create_add_node g x y = (if yvv = 0 then RefNode x else AddNode x y)"
        ...
        sorry
      then show ?thesis using cv3 ae yvv xn xv RefNode by auto
    next 
      case False
