...
theory ConditionalElimination2
  imports
...
lemma unwrap_bool:
  "val_to_bool (bool_to_val b) = b"
  by (induct b; auto)

lemma eq_not_lt:
  assumes "(is_IntVal32 xval \<and> is_IntVal32 yval) 
            \<or> (is_IntVal64 xval \<and> is_IntVal64 yval)"
  shows "val_to_bool (equal_value xval yval) \<longrightarrow> \<not>(val_to_bool (less_than_value xval yval))"
  by (induct xval; induct yval; auto simp: unwrap_bool) 

lemma eq_not_gt:
  assumes "(is_IntVal32 xval \<and> is_IntVal32 yval) 
            \<or> (is_IntVal64 xval \<and> is_IntVal64 yval)"
  shows "val_to_bool (equal_value xval yval) \<longrightarrow> \<not>(val_to_bool (less_than_value yval xval))"
  by (induct xval; induct yval; auto simp: unwrap_bool)

lemma lt_not_gt:
  assumes "(is_IntVal32 xval \<and> is_IntVal32 yval) 
            \<or> (is_IntVal64 xval \<and> is_IntVal64 yval)"
  shows "val_to_bool (less_than_value xval yval) \<longrightarrow> \<not>(val_to_bool (less_than_value yval xval))"
  by (induct xval; induct yval; auto simp: unwrap_bool)

lemma lt_not_eq:
  assumes "(is_IntVal32 xval \<and> is_IntVal32 yval) 
            \<or> (is_IntVal64 xval \<and> is_IntVal64 yval)"
  shows "val_to_bool (less_than_value xval yval) \<longrightarrow> \<not>(val_to_bool (equal_value xval yval))"
  by (induct xval; induct yval; auto simp: unwrap_bool)
...
lemma lt_not_eq_rev:
  assumes "(is_IntVal32 xval \<and> is_IntVal32 yval) 
            \<or> (is_IntVal64 xval \<and> is_IntVal64 yval)"
  shows "val_to_bool (less_than_value xval yval) \<longrightarrow> \<not>(val_to_bool (equal_value yval xval))"
  by (induct xval; induct yval; auto simp: unwrap_bool)
...
next
  ...
  using assms KnownFalse proof (induct x y imp rule: implies.induct)
    ...
    then obtain yval where yval: "g m \<turnstile> (kind g y) \<mapsto> yval"
      using eq_imp_less.prems(3)
      using evalDet by blast
    have eqeval: "g m \<turnstile> (IntegerEqualsNode x y) \<mapsto> equal_value xval yval"
      using eval.IntegerEqualsNode
      using xval yval
      by (metis IntegerEqualsNodeE eq_imp_less.prems(3) evalDet)
    have lesseval: "g m \<turnstile> (IntegerLessThanNode x y) \<mapsto> less_than_value xval yval"
      using eval.IntegerLessThanNode
      using xval yval
      by (metis IntegerLessThanNodeE eq_imp_less.prems(4) evalDet)
    have "(is_IntVal32 xval \<and> is_IntVal32 yval) 
            \<or> (is_IntVal64 xval \<and> is_IntVal64 yval)"
      by (metis IntegerLessThanNodeE evalDet lesseval xval yval)
    then have "val_to_bool (equal_value xval yval) \<longrightarrow> \<not>(val_to_bool (less_than_value xval yval))"
      using eq_not_lt by simp
    then show ?case
      using eqeval lesseval
      by (metis (full_types) "eq_imp_less.prems"(3) "eq_imp_less.prems"(4) evalDet)
  next
    case (eq_imp_less_rev x y)
    obtain xval where xval: "g m \<turnstile> (kind g x) \<mapsto> xval"
    ...
    have eqeval: "g m \<turnstile> (IntegerEqualsNode x y) \<mapsto> equal_value xval yval"
      using eval.IntegerEqualsNode
      using xval yval
      by (metis IntegerLessThanNodeE eq_imp_less_rev.prems(4) evalDet)
    have lesseval: "g m \<turnstile> (IntegerLessThanNode y x) \<mapsto> less_than_value yval xval"
      using eval.IntegerLessThanNode
      using xval yval
      by (metis IntegerLessThanNodeE eq_imp_less_rev.prems(4) evalDet)
    have "(is_IntVal32 xval \<and> is_IntVal32 yval) 
        \<or> (is_IntVal64 xval \<and> is_IntVal64 yval)"
      by (metis IntegerLessThanNodeE evalDet lesseval xval yval)
    then have "val_to_bool (equal_value xval yval) \<longrightarrow> \<not>(val_to_bool (less_than_value yval xval))"
      using eq_not_gt by simp
    then show ?case
      using eqeval lesseval
      by (metis eq_imp_less_rev.prems(3) eq_imp_less_rev.prems(4) evalDet)
  next
    case (less_imp_rev_less x y)
    obtain xval where xval: "g m \<turnstile> (kind g x) \<mapsto> xval"
    ...
    have lesseval: "g m \<turnstile> (IntegerLessThanNode x y) \<mapsto> less_than_value xval yval"
      using eval.IntegerLessThanNode
      using xval yval
      by (metis IntegerLessThanNodeE less_imp_rev_less.prems(4) evalDet)
    have revlesseval: "g m \<turnstile> (IntegerLessThanNode y x) \<mapsto> less_than_value yval xval"
      using eval.IntegerLessThanNode
      using xval yval
      by (metis IntegerLessThanNodeE less_imp_rev_less.prems(4) evalDet)
    have "(is_IntVal32 xval \<and> is_IntVal32 yval) 
        \<or> (is_IntVal64 xval \<and> is_IntVal64 yval)"
      by (metis IntegerLessThanNodeE evalDet lesseval xval yval)
    then have "val_to_bool (less_than_value xval yval) \<longrightarrow> \<not>(val_to_bool (less_than_value yval xval))"
      using lt_not_gt by simp
    then show ?case
      by (metis (full_types) evalDet less_imp_rev_less.prems(3,4) less_imp_rev_less.prems(3) lesseval revlesseval)
  next
    case (less_imp_not_eq x y)
    obtain xval where xval: "g m \<turnstile> (kind g x) \<mapsto> xval"
    ...
    have eqeval: "g m \<turnstile> (IntegerEqualsNode x y) \<mapsto> equal_value xval yval"
      using eval.IntegerEqualsNode
      using xval yval
      by (metis IntegerEqualsNodeE less_imp_not_eq.prems(4) evalDet)
    have lesseval: "g m \<turnstile> (IntegerLessThanNode x y) \<mapsto> less_than_value xval yval"
      using eval.IntegerLessThanNode
      using xval yval
      by (metis IntegerEqualsNodeE less_imp_not_eq.prems(4) evalDet)
    have "(is_IntVal32 xval \<and> is_IntVal32 yval) 
        \<or> (is_IntVal64 xval \<and> is_IntVal64 yval)"
      by (metis IntegerLessThanNodeE evalDet lesseval xval yval)
    then have "val_to_bool (less_than_value xval yval) \<longrightarrow> \<not>(val_to_bool (equal_value xval yval))"
      using lt_not_eq by simp
    then show ?case
      by (metis (full_types) eqeval evalDet less_imp_not_eq.prems(3,4) less_imp_not_eq.prems(3) lesseval)
  next
    case (less_imp_not_eq_rev x y)
    obtain xval where xval: "g m \<turnstile> (kind g x) \<mapsto> xval"
    ...
    then obtain yval where yval: "g m \<turnstile> (kind g y) \<mapsto> yval"
      ...
      using evalDet by blast
    have eqeval: "g m \<turnstile> (IntegerEqualsNode y x) \<mapsto> equal_value yval xval"
      using eval.IntegerEqualsNode
      using xval yval
      by (metis IntegerEqualsNodeE less_imp_not_eq_rev.prems(4) evalDet)
    have lesseval: "g m \<turnstile> (IntegerLessThanNode x y) \<mapsto> less_than_value xval yval"
      using eval.IntegerLessThanNode
      using xval yval
      by (metis IntegerEqualsNodeE less_imp_not_eq_rev.prems(4) evalDet)
    have "(is_IntVal32 xval \<and> is_IntVal32 yval) 
        \<or> (is_IntVal64 xval \<and> is_IntVal64 yval)"
      by (metis IntegerLessThanNodeE evalDet lesseval xval yval)
    then have "val_to_bool (less_than_value xval yval) \<longrightarrow> \<not>(val_to_bool (equal_value yval xval))"
      using lt_not_eq_rev by simp
    then show ?case
      by (metis (full_types) eqeval evalDet less_imp_not_eq_rev.prems(3,4) less_imp_not_eq_rev.prems(3) lesseval)
  next
    case (x_imp_x x1)
    then show ?case by simp
  next
...
inductive tryFold :: "IRNode \<Rightarrow> (ID \<Rightarrow> Stamp) \<Rightarrow> TriState \<Rightarrow> bool"
  where
  "\<lbrakk>always_distinct (stamps x) (stamps y)\<rbrakk> 
    \<Longrightarrow> tryFold (IntegerEqualsNode x y) stamps KnownFalse" |
  "\<lbrakk>never_distinct (stamps x) (stamps y)\<rbrakk> 
    \<Longrightarrow> tryFold (IntegerEqualsNode x y) stamps KnownTrue" |
  "\<lbrakk>stamps x = IntegerStamp isx;
    stamps y = IntegerStamp isy;
    upper_bound isx < lower_bound isy\<rbrakk> 
    \<Longrightarrow> tryFold (IntegerLessThanNode x y) stamps KnownTrue" |
  "\<lbrakk>stamps x = IntegerStamp isx;
    stamps y = IntegerStamp isy;
    lower_bound isx \<ge> upper_bound isy\<rbrakk> 
...
\<close>
lemma equals_never_undef:
  assumes "g m \<turnstile> IntegerEqualsNode x y \<mapsto> v"
  shows "v \<noteq> UndefVal"
  using assms proof -
  obtain xval where xval: "g m \<turnstile> kind g x \<mapsto> xval"
    using IntegerEqualsNodeE assms by auto
  obtain yval where yval: "g m \<turnstile> kind g y \<mapsto> yval"
    using IntegerEqualsNodeE assms by auto
  have bit_right: "(is_IntVal32 xval \<and> is_IntVal32 yval) 
        \<or> (is_IntVal64 xval \<and> is_IntVal64 yval)" (is "?b32 \<or> ?b64")
    using IntegerEqualsNodeE assms
    by (metis evalDet xval yval)
  have vdef: "v = equal_value xval yval"
    using xval yval assms IntegerEqualsNodeE
    by (metis evalDet)
  show ?thesis
  proof (cases ?b32)
    case True
    then show ?thesis using vdef apply (cases xval; cases yval; auto)
      using bool_to_val.simps
      by (metis (full_types) Value.distinct(1)) 
  next
    case False
    have ?b64 using False bit_right by auto
    then show ?thesis using vdef apply (cases xval; cases yval; auto)
      using bool_to_val.simps
      by (metis (full_types) Value.distinct(1)) 
  qed
...
lemma equal_value_false:
  assumes "(is_IntVal32 xval \<and> is_IntVal32 yval) 
        \<or> (is_IntVal64 xval \<and> is_IntVal64 yval)" (is "?b32 \<or> ?b64")
  assumes "xval \<noteq> yval"
  shows "(equal_value xval yval) = IntVal32 0"
  using assms equal_value.simps
proof (cases ?b32)
  case True
  obtain xv where xv: "xval = IntVal32 xv"
    using True is_IntVal32_def by auto
  obtain yv where yv: "yval = IntVal32 yv"
    using True is_IntVal32_def by auto
  have "xv \<noteq> yv" using assms xv yv by simp
  then have "equal_value xval yval = bool_to_val False"
    using equal_value.simps(1) is_IntVal32_def xv yv by simp
  then show ?thesis using bool_to_val.simps(2) by simp
next
  case False
  have b64: ?b64 using assms(1) False by simp
  obtain xv where xv: "xval = IntVal64 xv"
    using b64 is_IntVal64_def by auto
  obtain yv where yv: "yval = IntVal64 yv"
    using b64 is_IntVal64_def by auto
  have "xv \<noteq> yv" using assms xv yv by simp
  then have "equal_value xval yval = bool_to_val False"
    using equal_value.simps(1) is_IntVal32_def xv yv by simp
...
lemma equal_value_true:
  assumes "(is_IntVal32 xval \<and> is_IntVal32 yval) 
        \<or> (is_IntVal64 xval \<and> is_IntVal64 yval)" (is "?b32 \<or> ?b64")
  assumes "xval = yval"
  shows "(equal_value xval yval) = IntVal32 1"
  using assms equal_value.simps
proof (cases ?b32)
  case True
  obtain xv where xv: "xval = IntVal32 xv"
  ...
  obtain yv where yv: "yval = IntVal32 yv"
    using True is_IntVal32_def by auto
  have "xv = yv" using assms xv yv by simp
  then have "equal_value xval yval = bool_to_val True"
    using equal_value.simps(1) is_IntVal32_def xv yv by simp
  then show ?thesis using bool_to_val.simps(1) by simp
next
  case False
  have b64: ?b64 using assms(1) False by simp
  obtain xv where xv: "xval = IntVal64 xv"
...
lemma tryFoldIntegerEqualsAlwaysDistinct:
  ...
  assumes "always_distinct (stamps x) (stamps y)"
  shows "v = IntVal32 0"
  using assms eval.IntegerEqualsNode always_distinct_unequal
...
proof -
  ...
  then have "xval \<noteq> yval"
    using always_distinct_unequal assms(1,4) xval yval
    unfolding wf_stamp.simps
    by blast
  then have "(equal_value xval yval) = IntVal32 0"
