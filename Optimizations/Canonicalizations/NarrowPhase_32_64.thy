...
lemma narrow_reverse_pos:
  ...
  apply (simp add: neg1_shift_eq_mask signed_take_bit_eq_if_positive[OF pos])
proof (intro iffI)
  assume a: "konst = take_bit 7 val32"
(*  then have "konst = take_bit 8 val32"
    thm take_bit_Suc_from_most
  using assms take_bit_Suc_from_most 
*)
  then have 8: "and konst (mask 8) = konst"
    using mask_eq_iff_w2p
    by (smt (verit, ccfv_SIG) and_mask_lt_2p mask_eq_iff numeral_Bit0 numeral_less_iff one_eq_numeral_iff power_strict_increasing semiring_norm(81) take_bit_eq_mask)  
  then have "(signed_take_bit 7 revKonst) = konst"
    using 5 by simp
  then have 9: "(take_bit 7 revKonst) = konst"
    by (simp add: 4 a bit_and_iff bit_take_bit_iff signed_take_bit_eq_if_positive) 
(*
  then have "and revKonst (mask 8) = revKonst"
    by (simp add: "4" shiftl_def)
  then have 10: "and val32 (mask 8) = val32"
    using 8 9 a by fastforce
*)
  show "and konst (mask 8) = val32"

    using 9 10 a by fastforce
  next
    assume b: "and konst (mask 8) = val32"
    then have "take_bit 8 val32 = val32"
      by (metis and.right_idem take_bit_eq_mask)
    then have "take_bit 7 val32 = val32"
      using take_bit_suc pos by force 
    then have 8: "and konst (mask 8) = konst"
      using assms 
      print_facts
    then show "konst = take_bit 7 val32"
      using b 
    qed
...
lemma narrow_reverse_neg:
  ...
  shows "IntVal32 i32 = intval_narrow 32 8 (IntVal32 val32)
        \<longleftrightarrow> 
         IntVal32 (or i32 (- 1 << 8)) = (IntVal32 val32)"
  unfolding assms intval_narrow.simps narrow_helper.simps
  apply (simp add: neg1_shift_eq_mask signed_take_bit_eq_if_negative[OF neg])
proof (intro iffI)
  assume "i32 = - (64::32 word)"
  then show "or i32 (not (mask (8::nat))) = (192::32 word)"
    apply (code_simp; simp)
    (* now we have: 
           i32 = - (64::32 word) \<Longrightarrow> False
       i.e. a contradiction.  This is not provable.
    *)
    oops
