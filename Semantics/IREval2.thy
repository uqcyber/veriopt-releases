...
inductive
  ...
  "\<lbrakk>g m \<turnstile> (kind g x) \<mapsto> v\<rbrakk> 
    \<Longrightarrow> g m \<turnstile> (AbsNode x) \<mapsto> abs_value v" |

  NegateNode:
  "\<lbrakk>g m \<turnstile> (kind g x) \<mapsto> v\<rbrakk> 
    \<Longrightarrow> g m \<turnstile> (NegateNode x) \<mapsto> negate_value v" |
...
(* NOTE: if we use IntVal(bool_to_int(v1=v2)), then code generation does not work! *)
  ...
  "\<lbrakk>g m \<turnstile> (kind g x) \<mapsto> v1;
    ...
    (is_IntVal32 v1 \<and> is_IntVal32 v2)
    \<or> (is_IntVal64 v1 \<and> is_IntVal64 v2)\<rbrakk> 
    \<Longrightarrow> g m \<turnstile> (IntegerEqualsNode x y) \<mapsto> equal_value v1 v2" |
  ...
  "\<lbrakk>g m \<turnstile> (kind g x) \<mapsto> v1;
    ...
    (is_IntVal32 v1 \<and> is_IntVal32 v2)
    \<or> (is_IntVal64 v1 \<and> is_IntVal64 v2)\<rbrakk> 
    \<Longrightarrow> g m \<turnstile> (IntegerLessThanNode x y) \<mapsto> less_than_value v1 v2" |
...
(* Note that both branches are evaluated but only one is used.
  ...
  ConditionalNode:
  "\<lbrakk>g m \<turnstile> (kind g condition) \<mapsto> cond;
    g m \<turnstile> (kind g trueExp) \<mapsto> trueVal;
    g m \<turnstile> (kind g falseExp) \<mapsto> falseVal;
    val = (if (val_to_bool cond) then trueVal else falseVal)\<rbrakk> 
...
(* Note that v2 may evaluate to UnDef but is not used if v1 is true *)
  ...
  "\<lbrakk>g m \<turnstile> (kind g x) \<mapsto> v1;
    g m \<turnstile> (kind g y) \<mapsto> v2;
    val = (if (val_to_bool v1) then v1 else v2)\<rbrakk> 
    \<Longrightarrow> g m \<turnstile> (ShortCircuitOrNode x y) \<mapsto> val" |

  LogicNegationNode:
  "\<lbrakk>g m \<turnstile> (kind g x) \<mapsto> IntVal32 v1;
