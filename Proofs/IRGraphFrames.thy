...
theorem stay_same:
  ...
  assumes g1: "g1 m \<turnstile> (kind g1 nid) \<mapsto> v1"
  assumes wf: "wf_graph g1"
  shows "g2 m \<turnstile> (kind g2 nid) \<mapsto> v1"
proof -
  ...
  next
    ...
    then have "g2 m \<turnstile> (kind g2 child) \<mapsto> val"
  ...
  next
    ...
    then have "g2 m \<turnstile> (kind g2 nid) \<mapsto> abs_value v"
      using AbsNode.hyps(1) AbsNode.hyps(2) not_in_g
      by (metis AbsNode.hyps(3) AbsNode.prems(2) eval.AbsNode eval_in_ids kind_unchanged)
    then show ?case
      by (metis)
  next
  ...
  next
    ...
    have uy: "unchanged (eval_usages g1 y) g1 g2"
      by (metis IRNodes2.inputs_of_AddNode child_member_in child_unchanged member_rec(1) node.hyps(5) node.prems(1) node.prems(2))
  ...
  next
    case node: (IntegerEqualsNode m x v1 y v2 val)
    then have ux: "unchanged (eval_usages g1 x) g1 g2"
  ...
  next
    case node: (IntegerLessThanNode m x v1 y v2 val)
    then have ux: "unchanged (eval_usages g1 x) g1 g2"
  ...
  next
    case node: (ShortCircuitOrNode m x v1 y v2 val)
    then have ux: "unchanged (eval_usages g1 x) g1 g2"
    ...
    have y: "g1 m \<turnstile> (kind g1 y) \<mapsto> v2"
      using node.hyps(3) by blast
    have x2: "g2 m \<turnstile> (kind g2 x) \<mapsto> v1"
    ...
    have y2: "g2 m \<turnstile> (kind g2 y) \<mapsto> v2"
  ...
  next
    ...
    then have x:"g2 m \<turnstile> (kind g2 x) \<mapsto> IntVal32 v1"
  ...
  next
    case node:(ConditionalNode m condition cond trueExp trueVal falseExp falseVal val)
    have c: "condition \<in> inputs g1 nid"
      by (metis IRNodes2.inputs_of_ConditionalNode child_member_in member_rec(1) node.hyps(8) node.prems(1))
    then have "unchanged (eval_usages g1 condition) g1 g2"
      using child_unchanged node.prems(2) by blast
    then have cond: "g2 m \<turnstile> (kind g2 condition) \<mapsto> cond"
    ...
    then have trueVal: "g2 m \<turnstile> (kind g2 trueExp) \<mapsto> trueVal"
    ...
    then have ufalse: "unchanged (eval_usages g1 falseExp) g1 g2"
      using node.prems(2) child_unchanged by blast
    then have falseVal: "g2 m \<turnstile> (kind g2 falseExp) \<mapsto> falseVal"
      using node.hyps node f inp_in_g_wf wf by blast

    have "g2 m \<turnstile> (kind g2 nid) \<mapsto> val"
  ...
  next
    ...
    have x: "x \<in> inputs g1 nid"
      by (metis IRNodes2.inputs_of_RefNode RefNode.hyps(3) RefNode.prems(1) child_member_in member_rec(1))
  ...
  next
    case (InvokeNodeEval val m _ callTarget classInit stateDuring stateAfter nex)
    then show ?case
  ...
  next
    ...
    have obj: "obj \<in> inputs g1 nid"
      by (metis IRNodes2.inputs_of_IsNullNode IsNullNode.hyps(4) inputs.simps list.set_intros(1))
